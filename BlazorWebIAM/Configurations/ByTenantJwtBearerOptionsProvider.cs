...
using BlazorWebIAM.Client.Template;
...
namespace BlazorWebIAM.Configurations;
...
public class ByTenantJwtBearerOptionsProvider : IJwtBearerOptionsProvider
{
  ...
  public async Task<OpenIdConnectConfiguration?> GetOpenIdConfigurationAsync(JwtBearerOptions options, CancellationToken cancellationToken)
  {
    ...
    if (string.IsNullOrWhiteSpace(authority)) throw new ArgumentNullException(nameof(options));
  ...
  public void ConfigureOptions(JwtBearerOptions options)
  {
    ...
    if (tenantId is null || string.IsNullOrWhiteSpace(tenantId))
    ...
    if (tenantConfigSection is null || !tenantConfigSection.Exists())
    {
      tenantConfigSection = _configuration.GetSection(GetTenantConfigKey(TemplateConfigKey));
      if (tenantConfigSection is null)
    ...
    if (cacheDelayInSec is not null)
